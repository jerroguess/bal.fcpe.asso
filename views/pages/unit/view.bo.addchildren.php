 ...
 */
	...
	$strSection = $this->_request->getParamByKey('civilite_Saisie');
	$strTelephone = $this->_request->getParamByKey('nom2_Saisie');
	$strEmail = $this->_request->getParamByKey('prenom2_Saisie');
	...
	// Ouverture du cadre.
	// ******************************************
	add_cadre_open_form("addchildren", "soumettre", "formulairePrincipale"); 
	echo '<h1><img class="fcpe" alt="logo" src="public/images/fcpe.png">&nbsp;&nbsp;Ajouter un enfant</h1>';
    echo '<br/>';
	...
	if ($rdsTartanpion) {
		...
		while (!$rdsTartanpion->EOF) {
			...
			echo '<option value="' . $rdsTartanpion->fields['id'] . '">' . $rdsTartanpion->fields['nom'] . '</option>';
	...
	echo '<div id="exploration">';
		...
		if ($rdsExploration) {
			...
			while (!$rdsExploration->EOF) {
				...
				$strChecked = '';
				if ($iIdExploration1 == $rdsExploration->fields['id_exploration']) $strChecked = ' selected="selected" ';
		...
		if ($rdsExploration) {
			...
			while (!$rdsExploration->EOF) {
				...
				$strChecked = '';
				if ($iIdExploration2 == $rdsExploration->fields['id_exploration']) $strChecked = ' selected="selected" ';
		...
		if ($rdsExploration) {
			...
			while (!$rdsExploration->EOF) {
				...
				$strChecked = '';
				if ($iIdExploration3 == $rdsExploration->fields['id_exploration']) $strChecked = ' selected="selected" ';
	...
	echo '<br/>';
	echo '<br/>';
	echo '<button type="submit" id="btnLogin" class="btn btn-success">Ajouter</button>';
	echo '<br/>';
