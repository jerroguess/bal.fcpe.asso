...
class compressor {
	...
	**/
	function set_options() {
	   ...
	   $_SERVER['DOCUMENT_ROOT'] = $this->options['document_root'];
	   $this->view->set_paths();
	...
	**/	
	...
	foreach($matches[0] AS $link) {
		...
		preg_match_all("@(rel|media)=[\"'](.*?)[\"']@",$link,$variants,PREG_SET_ORDER);
	...
	**/	
	function do_compress($options,$source) {	
		...
		//Get the cache hash
		$cache_file = '_cmp_' . $options['save_name'] . '_' . md5(implode("_",$script_array).$datestring.implode("_",$options));
				...
				foreach($script_array AS $key=>$info) {	
					...
					if (file_exists($info['src'])) {
					   $contents .= file_get_contents($info['src']) . "\n";		
						...
						$contents = $this->convert_paths_to_absolute($contents,$info);
						}									   		   
	...
	**/
	...
	$relative_cachedir = str_replace($this->view->paths['full']['document_root'],"",$options['cachedir']);
	
	$newfile = "<" . $options['tag'] . " type=\"" . $options['type'] . "\" $options[src]=\"http://" . $_SERVER['HTTP_HOST'] . "/" . $relative_cachedir ."/$cache_file." . $options['ext'] . "\"";
	...
	**/		
	function get_script_array($source,$options) {	
		...
		if(!is_array($script_array)) { //Noting to do
		return $source;
		}
		...
		if(count($matches[1]) > 0) {
			...
			foreach($matches[1] AS $key=>$file) {
				...
				if(substr($file,0,1) != "/" && substr($file,0,5) != "http:") { //Not absolute
					...
					$absolute_path = "/" . str_replace($this->view->paths['full']['document_root'],"",$full_path_to_image . $file);								
					//$absolute_path = '../public/public/'. $file;
