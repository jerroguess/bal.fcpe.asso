...
class common_Text {
    ...
    /**
      ...
      @return	array
     */
    public static function arrayNonEmpty($array) {
    ...
    public static function splitWords($str) {
        ...
        $non_word = '\x{0000}-\x{002A}\x{002C}\x{002E}\x{002F}\x{003A}-\x{0040}\x{005b}-\x{0060}\x{007B}-\x{007E}\x{00A0}-\x{00BF}\s';

        if (preg_match_all('/([^' . $non_word . ']{3,})/msu', common_Html::clean($str), $match)) {
            foreach ($match[1] as $i => $v) {
                $current = strtolower($v);
    ...
    public static function detectEncoding($str) {
        return strtolower(mb_detect_encoding($str . ' ', 'UTF-8,ISO-8859-1,ISO-8859-2,ISO-8859-3,' .
